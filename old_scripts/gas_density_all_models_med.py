...
s2 = pynbody.load('../med_padoan_iso/med.01000')
s2.physical_units()
s2.g['n'] = s2.g['rho'].in_units('kg cm^-3')/(1.673*10**(-27))

s3 = pynbody.load('../med_semenov_iso/med.01000')
s3.physical_units()
s3.g['n'] = s3.g['rho'].in_units('kg cm^-3')/(1.673*10**(-27))

s4 = pynbody.load('../med_evans_iso/med.01000')
s4.physical_units()
...
plt.savefig('Gas_iso_com_all_med.pdf', bbox_inches='tight')
plt.clf()
