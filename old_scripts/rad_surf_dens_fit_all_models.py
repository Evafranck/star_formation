...
for k, sim in enumerate(path):
    ...
    axes[k].set_ylim(-1,5.4)
    axes[k].set_xlim(-1.5,14.9)
    ...
    axes[k].legend(fontsize=11, loc='upper center', bbox_to_anchor=(0.58, 0.845))
...
for k, sim in enumerate(path2):
    ...
    if not os.path.isfile(sim+'_surf_den_test.dat'):
        ...
        #print(g.fit_info['param_cov'])
    #plot the data
    axes[k+4].scatter(R_bins[13:],np.log10(surface_mass_density[13:]),marker='o', s=1.5, color='k',zorder=10, label = 'simulation data')
    ...
    axes[k+4].set_ylim(-1,5.4)
    axes[k+4].set_xlim(-1.5,14.9)
    axes[k+4].set_xlabel(r"R [kpc]", fontsize = 16)
    ...
    axes[k+4].legend(fontsize=11, loc='upper center', bbox_to_anchor=(0.58, 0.845))
...
plt.savefig('surface_density_fits_both.pdf', bbox_inches='tight')
