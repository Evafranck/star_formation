...
#x = [s.g['x'], s2.g['x']]
#y = [s.g['y'], s2.g['y']]
titlelist = ['a) Low resolved isolated simulation','b) Medium resolved isolated simulation','','','Initial star density in the high resolved isolated simulation','Initial gas density in the cosmological simulation', 'Initial star density in the cosmological simulation']
fig = plt.figure(figsize = (10,6.08))
gs0 = gd.GridSpec(2, 2, figure=fig, height_ratios = [1, 0.25], width_ratios = [1,1.058])
...
for n in range (2, 4):
    ...
    hist2, xbin, ybin = np.histogram2d(s2.s['x'], s2.s['y'],weights=s2.s['n'], bins=400, range = ((-50, 50), (-50,50)))
    hist = [hist1, hist2]
    im = ax.imshow(np.log10(hist[n-2]), extent=(-50,50,-50,50), cmap='CMRmap_r', transform = rot+base, vmin = -2, vmax = 6)
    if n == 2:
        ax.set_ylabel('y [kpc]', fontsize = 14)
    else:
        ax.set_yticklabels([])
