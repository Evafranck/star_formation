...
s = pynbody.load('../med_master_iso/med.01000')
s.physical_units()
new = f.LowPass('age', s.properties['time'].in_units('Gyr'))
s2 = s.s[new]
s.g['n'] = s.g['rho'].in_units('kg cm^-3')/(1.673*10**(-27))
s2.s['n'] = s2.s['rho'].in_units('kg cm^-3')/(1.673*10**(-27))

s3 = pynbody.load('../med_padoan_iso/med.01000')
s3.physical_units()
new = f.LowPass('age', s3.properties['time'].in_units('Gyr'))
s4 = s3.s[new]
s3.g['n'] = s3.g['rho'].in_units('kg cm^-3')/(1.673*10**(-27))
s4.s['n'] = s4.s['rho'].in_units('kg cm^-3')/(1.673*10**(-27))

key = [s.g['n'], s3.g['n'], s2.s['n'], s4.s['n'], s.g['n'], s3.g['n'], s2.s['n'], s4.s['n']]
x = [s.g['x'], s3.g['x'], s2.s['x'], s4.s['x'], s.g['x'], s3.g['x'], s2.s['x'], s4.s['x']]
y = [s.g['y'], s3.g['y'], s2.s['y'], s4.s['y'], s.g['y'], s3.g['y'], s2.s['y'], s4.s['y']]
titlelist = ['a) Threshold-based model'+'\n'+'Gas density', 'b) Padoan (2012)' + '\n' + 'Gas density','c) Threshold-based model'+'\n'+ 'Newly formed star density', 'd) Padoan (2012)'+'\n'+'Newly formed star density', '', '', '', '']
...
for n in range(4):
    ...
    im = ax.imshow(np.log10(hist), extent=(-50,50,-50,50), cmap='CMRmap_r', vmin = -8, vmax = 5)
    if n == 3:
...
for n in range(4, 8):
    ...
    hist, xbin, ybin = np.histogram2d(x[n], y[n],weights=key[n], bins=600, range = ((-50, 50), (-50,50)))
    im = ax.imshow(np.log10(hist), extent=(-50,50,-50,50), cmap='CMRmap_r', transform = rot+base, vmin = -7.9, vmax = 4.9)
...
plt.savefig('Gas_stars_iso_res_com_both.pdf', bbox_inches='tight')
plt.clf()
